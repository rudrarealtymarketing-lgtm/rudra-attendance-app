...
import { 
  ...
  Briefcase, Phone, Mail, Award, CheckCircle, TrendingUp, AlertCircle
} from 'lucide-react';
...
export const DirectorPortal: React.FC<DirectorPortalProps> = ({ 
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (usersRes.ok) {
        const uList: User[] = await usersRes.json();
        setUsers(uList);
      }
  ...
  // Attendance records for selected date (strictly staff only)
  ...
  // Dashboard KPI Stats (Strictly excludes Admin and Director accounts)
  ...
  // Filtered Master Attendance Records based on Date Range, Site, Employee, Status filter, and Search
  ...
  // List of absentees for the selected date (staff with no punch logged)
  ...
  const handleApproveRequest = async (id: number) => {
    const comment = prompt('Enter director note for approval (Optional):', 'Approved by Director');
    const approver = currentUser.name ? `${currentUser.name} (Director)` : 'Director';
    try {
      const res = await fetch(`/api/attendance/requests/${id}/approve`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ adminComment: comment || 'Approved by Director', actionedBy: approver })
      });
      if (res.ok) {
  ...
  const handleRejectRequest = async (id: number) => {
    const comment = prompt('Enter reason for declining request:', 'Declined by Director');
    const rejector = currentUser.name ? `${currentUser.name} (Director)` : 'Director';
    try {
      const res = await fetch(`/api/attendance/requests/${id}/reject`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ adminComment: comment || 'Declined by Director', actionedBy: rejector })
      });
      if (res.ok) {
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-800 dark:text-slate-100 flex flex-col justify-between pb-24 px-3 sm:px-4 pt-3 max-w-5xl mx-auto font-sans transition-colors">
      ...
      <div>
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 mb-3.5">
          <div className="flex items-center gap-2.5">
            <div className="w-10 h-10 bg-indigo-50 dark:bg-indigo-950/60 text-indigo-600 dark:text-indigo-400 rounded-2xl flex items-center justify-center border border-indigo-200 dark:border-indigo-800 shadow-xs">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400">
                Strategic Attendance Oversight & Timesheet Analytics
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              title="Change Director Password"
              className="flex items-center gap-1 px-3 py-1.5 bg-indigo-50 dark:bg-indigo-950/60 border border-indigo-200 dark:border-indigo-800 text-indigo-700 dark:text-indigo-300 hover:bg-indigo-100 rounded-xl text-xs font-semibold shadow-xs cursor-pointer transition-all active:scale-95"
            >
            ...
            <button
              ...
              className="flex items-center gap-1 px-3.5 py-1.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-xs font-semibold shadow-xs active:scale-95 transition-all cursor-pointer"
            >
        ...
        {/* Tab Navigation: Dashboard | Master Attendance | Requests */}
        <div className="flex bg-white dark:bg-slate-900 border border-slate-200/90 dark:border-slate-800 p-1 rounded-2xl mb-4 shadow-xs gap-1">
          <button
            onClick={() => { setActiveTab('dashboard'); setSelectedStaffUser(null); }}
            className={`py-2 px-3 rounded-xl text-xs font-bold transition-all flex-1 text-center cursor-pointer ${
              activeTab === 'dashboard'
          ...
          <button
            onClick={() => { setActiveTab('attendance'); setSelectedStaffUser(null); }}
            className={`py-2 px-3 rounded-xl text-xs font-bold transition-all flex-1 text-center cursor-pointer ${
          ...
          >
            📋 Master Attendance {activeFilteredEmployee ? `(${activeFilteredEmployee.name})` : ''}
          </button>

          <button
            onClick={() => { setActiveTab('requests'); setSelectedStaffUser(null); }}
            className={`py-2 px-3 rounded-xl text-xs font-bold transition-all flex-1 text-center cursor-pointer ${
              activeTab === 'requests'
                ? 'bg-indigo-600 text-white shadow-xs'
                : 'text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white'
        ...
        {activeTab === 'dashboard' && (
          <div className="space-y-4">
            ...
            {/* Prominent Employee Quick Search Bar */}
            <div className="bg-gradient-to-br from-indigo-900/10 via-blue-900/5 to-slate-900/10 dark:from-indigo-950/40 dark:via-slate-900 dark:to-slate-900 border border-indigo-200/80 dark:border-indigo-900/60 rounded-3xl p-3.5 shadow-xs">
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-1.5 text-xs font-bold text-indigo-950 dark:text-indigo-200">
                  <Search className="w-4 h-4 text-indigo-600 dark:text-indigo-400" />
                  <span>Search Staff / View Timesheet Instantly</span>
                </div>
                <span className="text-[10px] text-slate-400">Type Name, ID, or Designation</span>
              </div>
              <div className="relative">
                <input
                  ...
                  placeholder="Search by Employee Name, ID (e.g. EMP-1001), Site, or Role..."
                  className="w-full pl-9 pr-8 py-2.5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl text-xs font-medium text-slate-800 dark:text-white placeholder:text-slate-400 focus:ring-2 focus:ring-indigo-500 shadow-inner"
                />
            ...
            {/* Global Filter Bar: Site Selection + Date Picker */}
            <div className="bg-white dark:bg-slate-900 border border-slate-200/90 dark:border-slate-800 rounded-3xl p-3 shadow-xs flex flex-wrap items-center justify-between gap-2.5">
              ...
              <div className="flex items-center gap-2 flex-1 min-w-[200px]">
                ...
                >
                  <option value="All">All Construction Sites & Branches</option>
                  {sites.map(s => (
              ...
              {/* Single Date Picker for Today's View */}
              <div className="flex items-center gap-2">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5">
              ...
              <div 
                ...
                className="bg-white dark:bg-slate-900 border border-indigo-200 dark:border-indigo-900/60 hover:border-indigo-400 dark:hover:border-indigo-600 rounded-2xl p-3.5 shadow-xs text-center cursor-pointer group active:scale-98 transition-all"
                title="Click to view complete employee list in Master Attendance"
              >
                <div className="flex items-center justify-center gap-1 mb-0.5">
                  <Users className="w-3.5 h-3.5 text-indigo-600" />
                  <p className="text-[10px] text-indigo-600 font-bold uppercase">Total Staff</p>
                </div>
                <p className="text-2xl sm:text-3xl font-extrabold text-slate-900 dark:text-white mt-0.5 group-hover:text-indigo-600 transition-colors">
                  {dashboardStats.totalStaff}
                </p>
                <p className="text-[10px] text-slate-400 mt-1 flex items-center justify-center gap-0.5 group-hover:text-indigo-500">
            ...
            {/* Searchable Staff Directory & Live Punch List */}
            <div className="bg-white dark:bg-slate-900 border border-slate-200/90 dark:border-slate-800 rounded-3xl p-4 shadow-xs">
              <div className="flex items-center justify-between mb-3">
                <div>
                  ...
                  <p className="text-[11px] text-slate-400">Click any employee to open their full timesheet & history</p>
                </div>
                <span className="text-[11px] font-semibold text-indigo-600 bg-indigo-50 dark:bg-indigo-950/60 px-2.5 py-1 rounded-xl">
                ...
                {dashboardFilteredUsers.map(user => {
                  ...
                  return (
                    <div
                      ...
                      className="p-3 bg-slate-50/80 dark:bg-slate-800/60 hover:bg-indigo-50/70 dark:hover:bg-indigo-950/40 hover:border-indigo-300 dark:hover:border-indigo-700 border border-slate-200/80 dark:border-slate-700 rounded-2xl flex items-center justify-between cursor-pointer transition-all group"
                    >
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded-xl bg-gradient-to-tr from-indigo-600 to-blue-600 text-white font-bold text-xs flex items-center justify-center overflow-hidden shadow-xs shrink-0">
                          {user.avatar_url ? (
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span className="p-1 rounded-lg bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 text-indigo-600 dark:text-indigo-400 opacity-80 group-hover:opacity-100 transition-all">
        ...
        {/* ===================== TAB 2: MASTER ATTENDANCE (FULL CALENDAR & MULTI-FILTER) ===================== */}
        {activeTab === 'attendance' && (
          <div className="space-y-4">
            
            {/* Active Single-User Filter Banner */}
            {activeFilteredEmployee && (
              <div className="bg-indigo-50 dark:bg-indigo-950/60 border border-indigo-200 dark:border-indigo-800 rounded-2xl p-3 flex items-center justify-between">
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    <div className="flex items-center gap-1.5">
                      ...
                      <span className="text-[10px] font-mono px-1.5 py-0.2 bg-white dark:bg-slate-900 text-indigo-600 rounded border border-indigo-200">
                        {activeFilteredEmployee.registration_id}
                      </span>
                    </div>
                    <p className="text-[10px] text-indigo-600/80 dark:text-indigo-300">
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="px-2.5 py-1 bg-white dark:bg-slate-900 border border-indigo-200 dark:border-indigo-800 text-indigo-700 dark:text-indigo-300 rounded-xl text-xs font-semibold hover:bg-indigo-100"
                  >
                    View Details
                  </button>
                  <button
            ...
            {/* Filter Control Box: Date Range Calendar + Site Filter + Employee Filter */}
            <div className="bg-white dark:bg-slate-900 border border-slate-200/90 dark:border-slate-800 rounded-3xl p-4 shadow-xs space-y-3.5">
              ...
              {/* Filter Row 1: Site Dropdown & Employee Dropdown */}
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <select
                    value={selectedSite}
                    onChange={(e) => setSelectedSite(e.target.value)}
                    className="w-full p-2 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-xs font-semibold text-slate-800 dark:text-slate-200 focus:ring-2 focus:ring-indigo-500"
                  >
              ...
              {/* Filter Row 2: Status Type Filter (All, On-Time, Late, Absentees, Leave) */}
              ...
              {/* Filter Row 3: Full Date Range Calendar Pickers */}
                ...
                <div className="flex flex-wrap gap-1.5 pt-1">
                  {[
                    ...
                    <button
                      ...
                      className="px-2.5 py-1 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg text-[10px] font-semibold text-slate-700 dark:text-slate-300 hover:bg-indigo-50 dark:hover:bg-indigo-950/50 hover:border-indigo-300 transition-all cursor-pointer"
                    >
                      {preset.label}
                    </button>
                  ))}
              ...
              <div className="flex flex-wrap items-center justify-between gap-2 pt-1">
                <div className="text-xs text-slate-500">
                  Total Matching Records: <strong className="text-indigo-600 dark:text-indigo-400 font-bold">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button
                    onClick={handleExportPDF}
                    className="px-3.5 py-1.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-xs font-semibold flex items-center gap-1.5 shadow-xs active:scale-95 transition-all cursor-pointer"
                  >
                    <Download className="w-3.5 h-3.5" />
              ...
              {attendanceStatusFilter === 'ABSENT' ? (
                absentStaffList.length === 0 ? (
                  <div className="py-12 text-center text-emerald-600 text-xs bg-emerald-50 dark:bg-emerald-950/30 rounded-2xl border border-emerald-200 dark:border-emerald-800">
                    No absentees! 100% staff attendance logged for {selectedDate}.
                  </div>
                ) : (
                  ...
                  return (
                    ...
                    >
                      <div className="flex justify-between items-start">
                        <div className="flex items-center gap-2.5">
                          <div className="w-9 h-9 rounded-xl bg-indigo-600/10 text-indigo-600 dark:text-indigo-400 font-bold flex items-center justify-center border border-indigo-200 dark:border-indigo-900/60">
                      ...
                      <div className="bg-slate-50 dark:bg-slate-800/60 p-3 rounded-2xl border border-slate-100 dark:border-slate-800 space-y-1.5">
                        ...
                        <div className="flex flex-wrap items-center justify-between gap-2 pt-0.5">
                          <p className="text-slate-700 dark:text-slate-300">
                            Type: <strong className="text-indigo-600 dark:text-indigo-400 font-semibold uppercase">{displayType}</strong>
                          </p>
                          <p className="text-slate-700 dark:text-slate-300">
                        ...
                        {/* Decision Details if already actioned */}
                        {!isPending && (
      ...
      {/* Drill-down Modal for Employee Detailed Information & History */}
      {selectedStaffUser && (
        <div className="fixed inset-0 z-50 bg-slate-900/70 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-5 max-w-md w-full max-h-[88vh] flex flex-col shadow-2xl">
            ...
            <div className="flex items-center justify-between pb-3 border-b border-slate-100 dark:border-slate-800">
              <div className="flex items-center gap-3">
                <div className="w-12 h-12 rounded-2xl bg-gradient-to-tr from-indigo-600 to-blue-600 text-white flex items-center justify-center font-bold text-base shadow-xs">
                ...
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[10px] font-mono px-2 py-0.5 bg-indigo-50 dark:bg-indigo-950/60 text-indigo-700 dark:text-indigo-300 font-bold rounded-md border border-indigo-200 dark:border-indigo-800">
            ...
            <div className="pt-3 border-t border-slate-100 dark:border-slate-800 flex flex-col gap-2">
              <button
                ...
                className="w-full py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-xs font-semibold flex items-center justify-center gap-1.5 shadow-xs cursor-pointer"
              >
      ...
      {/* Hidden Print Container for Clean Native Printing */}
      <div id="printable-attendance-report" className="hidden">
        <div className="p-6">
          <div className="border-b-2 border-slate-900 pb-4 mb-4">
            ...
            <h2 className="text-sm font-semibold text-slate-600 uppercase tracking-wider">Official Attendance & Timesheet Report</h2>
      ...
      <div className="text-center pt-5 pb-1">
        <p className="text-[11px] text-slate-400 dark:text-slate-500 font-normal">
          Executive Timesheet Management • <span className="text-slate-700 dark:text-slate-300 font-semibold">Rudra Infra World</span>
        </p>
      </div>
