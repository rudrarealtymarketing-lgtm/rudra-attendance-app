...
export const AdminSitesTab: React.FC<AdminSitesTabProps> = ({
  ...
  const [editLat, setEditLat] = useState(23.0225);
  const [editLng, setEditLng] = useState(72.5714);
  ...
  const openEditModal = (s: Site) => {
    ...
    setEditLat(s.latitude || 23.0225);
    setEditLng(s.longitude || 72.5714);
    setEditRadius(s.geofence_radius || s.radius || 150);
  };
    ...
    try {
      const res = await fetch('/api/super_admin/sites', {
        ...
        body: JSON.stringify({
          ...
          geofence_radius: Number(radius) || 150
        })
      });
    ...
    try {
      const res = await fetch(`/api/super_admin/sites/${editingSite.id}`, {
        ...
        body: JSON.stringify({
          ...
          geofence_radius: Number(editRadius) || 150
        })
      });
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200/90 dark:border-slate-800 rounded-3xl p-4 shadow-xs space-y-2">
        ...
        <SiteGeofenceMap
          sites={sites}
          latitude={sites[0]?.latitude || 23.0225}
          longitude={sites[0]?.longitude || 72.5714}
          radius={sites[0]?.geofence_radius || 150}
          height="280px"
        />
      </div>
          ...
          >
            <div className="flex justify-between items-start">
              <div>
                ...
                <p className="text-slate-500 text-[11px] mt-0.5">{site.address || 'Ahmedabad, Gujarat'}</p>
              </div>
              <span className="px-2.5 py-1 bg-amber-50 dark:bg-amber-950/40 text-amber-700 dark:text-amber-300 border border-amber-200 dark:border-amber-800 rounded-full text-[10px] font-bold">
                {site.geofence_radius || site.radius || 150}m Geofence
              </span>
            </div>
            ...
            <div className="bg-slate-50 dark:bg-slate-800/60 p-2.5 rounded-2xl border border-slate-100 dark:border-slate-800 space-y-1 text-[11px] font-mono text-slate-600 dark:text-slate-300">
              <p>GPS: {site.latitude?.toFixed(5) || '23.0225'}, {site.longitude?.toFixed(5) || '72.5714'}</p>
              <p>Radius: {site.geofence_radius || site.radius || 150} meters</p>
            </div>

            <div className="flex items-center justify-between pt-1 border-t border-slate-100 dark:border-slate-800">
              <button
                onClick={() => openEditModal(site)}
                className="px-2.5 py-1 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 text-slate-700 dark:text-slate-300 rounded-lg text-[11px] font-semibold flex items-center gap-1 transition-colors"
              >
              ...
              <button
                ...
                className="p-1.5 text-rose-600 hover:bg-rose-50 dark:hover:bg-rose-950/40 rounded-lg text-[11px] font-semibold transition-colors"
                title="Delete Site"
              >
        ...
        <div className="fixed inset-0 z-50 bg-slate-900/70 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-5 max-w-lg w-full shadow-2xl space-y-3.5 text-xs max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleCreateSite} className="space-y-3">
              {/* Satellite Interactive Map Picker */}
              <div>
              ...
              <div className="grid grid-cols-2 gap-2.5">
                <div>
                  ...
                  <input
                    ...
                    placeholder="e.g. Rudra Sky High"
        ...
        <div className="fixed inset-0 z-50 bg-slate-900/70 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-5 max-w-lg w-full shadow-2xl space-y-3.5 text-xs max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center pb-2 border-b border-slate-100 dark:border-slate-800">
              ...
              <button onClick={() => setEditingSite(null)} className="p-1 text-slate-400 hover:text-slate-600">
                <X className="w-5 h-5" />
              </button>
            </div>
          ...
          <div className="bg-white dark:bg-slate-900 border border-rose-200 dark:border-rose-800 rounded-3xl p-5 max-w-xs w-full shadow-2xl text-center space-y-2">
            ...
            <div className="flex gap-2 pt-2">
              ...
              <button
                ...
                className="w-1/2 py-2 bg-rose-600 hover:bg-rose-700 text-white rounded-xl text-xs font-bold shadow-md"
              >
                Yes, Delete
              </button>
            </div>
